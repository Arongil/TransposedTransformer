...
with open(os.path.join(out_dir, "config.json"), "r") as f:
    config = ConfigNode(**json.load(f))

# build stoi and itos dicts
with open(config.data_filepath, "r") as f:
    data = f.read()
...
config.model = ConfigNode(**config.model)
config.model.vocab_size = vocab_size
model = TT(config.model)

# load the trained state dict
ckpt_path = os.path.join(out_dir, 'model.pt') # 'ckpt.pt')
